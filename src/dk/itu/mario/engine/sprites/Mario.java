...
public class Mario extends Sprite {
	...
	public void move() {
		...
		if (keys[KEY_JUMP] || (jumpTime < 0 && !onGround && !sliding)) {
			if (jumpTime < 0) {
				...
				world.sound.play(Art.samples[Art.SAMPLE_MARIO_JUMP], this, 1,
						1, 1);
				xJumpSpeed = 0;
				yJumpSpeed = -1.9f;
				jumpTime = 7;
		...
		if ((!keys[KEY_LEFT] && !keys[KEY_RIGHT]) || ducking || ya < 0
				|| onGround) {
			sliding = false;
		}

		if (keys[KEY_SPEED] && canShoot && Mario.fire
				&& world.fireballsOnScreen < 2) {
			world.sound.play(Art.samples[Art.SAMPLE_MARIO_FIREBALL], this, 1,
					1, 1);
		...
		if (sliding) {
			for (int i = 0; i < 1; i++) {
				world.addSprite(new Sparkle((int) (x + Math.random() * 4 - 2)
						+ facing * 8, (int) (y + Math.random() * 4) - 24,
		...
		// if(x > world.level.xExit * 16 && ! world.level.flipped || x <
		// world.level.xExit * 16 && world.level.flipped )
			...
			if (xa > 3 || xa < -3) {
				for (int i = 0; i < 3; i++) {
					world.addSprite(new Sparkle(
							(int) (x + Math.random() * 8 - 4), (int) (y + Math
									.random() * 4),
	...
	private boolean isBlocking(float _x, float _y, float xa, float ya) {
		...
		if (((Level.TILE_BEHAVIORS[block & 0xff]) & Level.BIT_PICKUPABLE) > 0) {
			...
			Mario.getCoin();
			world.sound.play(Art.samples[Art.SAMPLE_GET_COIN],
					new FixedSoundSource(x * 16 + 8, y * 16 + 8), 1, 1, 1);
			world.level.setBlock(x, y, (byte) 0);
			for (int xx = 0; xx < 2; xx++)
				for (int yy = 0; yy < 2; yy++)
					world.addSprite(new Sparkle(x * 16 + xx * 8
	...
	public void getHurt(Sprite sprite) {
		...
		if (large) {
			world.paused = true;
			powerUpTime = -3 * 6;
			world.sound.play(Art.samples[Art.SAMPLE_MARIO_POWER_DOWN], this, 1,
					1, 1);
			if (fire) {
	...
	public void die() {
		...
		// Updates death if using MyLevel
		Level level = world.level;
		if (level instanceof MyLevel) {
			((MyLevel) level).death((int) x / 16);
		}
	...
	public void getFlower() {
		...
		if (!fire) {
			world.paused = true;
			powerUpTime = 3 * 6;
			world.sound.play(Art.samples[Art.SAMPLE_MARIO_POWER_UP], this, 1,
					1, 1);
	...
	public static void get1Up() {
		instance.world.sound.play(Art.samples[Art.SAMPLE_MARIO_1UP], instance,
				1, 1, 1);
		lives++;
		if (lives == 99) {
			lives = 99;
